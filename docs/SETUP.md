...
Open `App.xcworkspace` in XCode. Open the project settings and change the team the app belongs to. Edit `.env` with the new target name you just set as well as all of the other values such as bundle identifier and app name. Run command: `./set_environment.rb`. This will rename the app, set bundle identifier, and a few other things. 

Compile the app in XCode and see if it builds. 
