...
# Script to check for (1) duplicate entries, (2) non-used entries, (3) undefined entries in your localized strings files. 

# From: https://buildingvts.com/clean-ios-localizable-files-8b910413b985
# https://github.com/ginowu7/CleanLocalizableExample

project_name="iOSBlanky"
development_file="./$project_name/en.lproj/Localizable.strings"

es_file_not_found=10
es_duplicates=9
es_match=8
es_not_included=7

if [ ! -f $development_file ]; then
    echo "Localization strings file not found. Cannot lint without file!"
	exit $es_file_not_found
fi
...
keys_not_included() {
	...
	while read key; do
		if [[ $base_keys != *$key* ]]; then
			...
			echo "error: \"$key\" not defined in file: $1"
